document.addEventListener('DOMContentLoaded', () => {
    ...
    const loadingSection = document.getElementById('loading-bar');
    let progress = 0;

    if (progressBar) {
        const progressInterval = setInterval(() => {
            progress += 2; // Slower increment for better visibility
            
            if (progress >= 100) {
                progress = 100;
                clearInterval(progressInterval);
                progressBar.textContent = '100% - Store Ready!';
                
                // Keep it visible for 3 seconds so you can actually see it's finished
                setTimeout(() => {
                    loadingSection.style.transition = "opacity 0.5s";
